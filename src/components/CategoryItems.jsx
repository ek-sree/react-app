...
const CategoryItems = ({ items, showAddButton }) => {
      ...
      {items.map((item) => (
        <div key={item?.card?.info?.id} className="border-b-2 pb-6 text-left pt-3">
          <div className="flex justify-between">
            ...
            {showAddButton && (<div className="flex justify-end absolute">
               <button className="p-2 ml-[620px] mt-[76px] bg-slate-50 rounded-lg shadow-xl mx-10 w-18 h-18 hover:scale-90 hover:ease-in duration-500 " onClick={()=>handleClick(item)}>
                ADD +
              </button>
            </div>
            )}
